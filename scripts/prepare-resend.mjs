...
const headers = ["email", "first_name", "last_name", "customer_status", "engagement_tier", "last_meaningful_activity_at", "source_portals", "consent_evidence_quality", "migration_cohort"];
const escape = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
const rows = selected.map((record) => [record.email, record.first_name, record.last_name, record.customer_status, record.engagement_tier, record.engagement.last_click_at || record.engagement.last_open_at || record.engagement.last_seen_at, record.source_portals.join(";"), record.consent.evidence_quality, mode].map(escape).join(","));
