...
if (!["free", "free-prospects", "pro"].includes(mode)) throw new Error("Use --mode=free, --mode=free-prospects, or --mode=pro");
...
const rankingPath = path.join(root, mode === "free-prospects" ? "free-prospect-ranking" : "free-ranking", "selected.json");
const selected = mode === "free" || mode === "free-prospects"
  ? JSON.parse(await readFile(rankingPath, "utf8"))
  : eligible;
const cap = mode === "free" || mode === "free-prospects" ? 1000 : 5000;
...
const destination = path.join(root, mode === "free-prospects" ? "free-prospect-import" : `${mode}-import`);
...
const manifest = { id: `resend-${mode}-${new Date().toISOString().replace(/[:.]/g, "-")}`, mode, sourceLedger: ledgerDir, rankingSource: mode === "free" || mode === "free-prospects" ? rankingPath : null, audienceCount: selected.length, audienceHash: sha256(selected.map((record) => record.email)), csvSha256: sha256(csv), approvalStatus: "pending", createdAt: new Date().toISOString() };
